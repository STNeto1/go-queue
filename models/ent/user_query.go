...
func (uq *UserQuery) loadQueues(ctx context.Context, query *QueueQuery, nodes []*User, init func(*User), assign func(*User, *Queue)) error {
	...
	query.Where(predicate.Queue(func(s *sql.Selector) {
		s.Where(sql.InValues(user.QueuesColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_queues
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_queues" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_queues" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
