...
func (uq *UserQuery) QueryQueues() *QueueQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withQueues: uq.withQueues.Clone(),
...
func (uq *UserQuery) WithQueues(opts ...func(*QueueQuery)) *UserQuery {
	query := &QueueQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withQueues; query != nil {
		if err := uq.loadQueues(ctx, query, nodes,
			func(n *User) { n.Edges.Queues = []*Queue{} },
			func(n *User, e *Queue) { n.Edges.Queues = append(n.Edges.Queues, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadQueues(ctx context.Context, query *QueueQuery, nodes []*User, init func(*User), assign func(*User, *Queue)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.QueuesTable)
		s.Join(joinT).On(s.C(queue.FieldID), joinT.C(user.QueuesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.QueuesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.QueuesPrimaryKey[0]))
